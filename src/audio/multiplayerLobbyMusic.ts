...
export const MULTIPLAYER_LOBBY_TRACK_URL = `${import.meta.env.BASE_URL}audio/music/menu/multiplayer_lobby.mp3`;
...
async function decodeMultiplayerLobbyTrack(ctx: AudioContext): Promise<AudioBuffer> {
  if (decodedTrack) return decodedTrack;

  const res = await fetch(MULTIPLAYER_LOBBY_TRACK_URL);
  if (!res.ok) throw new Error(`Failed to load multiplayer lobby music track: ${MULTIPLAYER_LOBBY_TRACK_URL}`);
...
async function playMultiplayerLobbyTrack(token: number): Promise<void> {
  const ctx = getAudioContext();
  try {
    const decoded = await decodeMultiplayerLobbyTrack(ctx);
    ...
    console.warn('[Alchemy] Multiplayer lobby music track failed to play.', err);
  }
}

export function startMultiplayerLobbyMusic(): void {
  if (isPlaying) return;
  isPlaying = true;
  loadToken += 1;
  const token = loadToken;
  void playMultiplayerLobbyTrack(token);
}

export function stopMultiplayerLobbyMusic(): void {
  if (!isPlaying) return;
  isPlaying = false;
  loadToken += 1;
  stopCurrentTrack();
