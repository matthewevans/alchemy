...
export function LearningOnboardingModal({
  ...
  const turnOnRef = useRef<HTMLButtonElement | null>(null);
  const logoWordmarkSrc = `${import.meta.env.BASE_URL}logo_wordmark.webp`;
  const dialogRef = useDialogA11y({
    open,
    closeOnEscape: false,
    initialFocusRef: turnOnRef,
  });
          ...
          >
            ...
            <div className="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-2">
              <button
                ...
                onClick={() => onComplete(false, ageRange)}
              >
                Not now
              </button>
              <button
                ...
                onClick={() => onComplete(true, ageRange)}
              >
                Turn On Learning Challenges
              </button>
            </div>
          </motion.div>
        </motion.div>
