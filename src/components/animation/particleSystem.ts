...
export class ParticleSystem {
  ...
  private loop = () => {
    // Stop the loop when there's no work — it restarts on next emit/addEffect
  ...
  private updateEffects(now: number) {
    for (let i = this.effects.length - 1; i >= 0; i--) {
      ...
      if (t >= 1) {
        ...
        // Swap with last and pop — O(1) removal (order doesn't matter for effects)
  ...
  private draw(now: number) {
    ...
    // Draw active effects (projectile bodies, shockwaves via draw callbacks)
