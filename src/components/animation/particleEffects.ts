...
export function emitExplosion(system: ParticleSystem, x: number, y: number, element?: Element) {
  ...
  system.addEffect({
    ...
    update() {},
    draw(t, ctx) {
      // Central flash — bright white that expands and fades
...
// ─── Effect: Keyword Trigger (brief element flash) ───
...
export function emitKeywordFlash(system: ParticleSystem, x: number, y: number, element?: Element) {
  ...
  system.emit([
    ...
    ...coreSparkles(x, y, 3, [20, 50], { glow: 12, sizeRange: [3, 5] }),
  ]);
  ...
  system.addEffect({
    ...
    update() {},
    draw(t, ctx) {
      const alpha = (1 - easeOutCubic(t)) * 0.6;
...
export function emitSummonBurst(system: ParticleSystem, x: number, y: number, element?: Element) {
  ...
  system.addEffect({
    ...
    update() {},
    draw(t, ctx) {
      // Vertical energy column — narrow pillar of light that rises and fades
...
// ─── Effect: Block Clash (sparks at midpoint of block link) ───
